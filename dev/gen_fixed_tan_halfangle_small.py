...
def emit(o, n):
    wn = n + 1
    o("static void")
    o("_fixed_tan_halfangle_%d(nn_ptr ysin, nn_ptr ycos, nn_ptr ytan," % n)
    o("    nn_srcptr x)")
    o("{")
    o("    slong num, nc;")
    ...
    o("    _fixed_tan_rs_opt_%d(u, t);          /* u = tan(t') */" % n)
    o("")
...
def main():
    ...
    lines = []
    o = lines.append
    o("/* Register half-angle reconstruction for n = %d..%d." % (NMIN, NMAX))
    o("   GENERATED by dev/gen_fixed_tan_halfangle_small.py -- do not")
    o("   edit directly. */")
    o("")
    for n in range(NMIN, NMAX + 1):
        emit(o, n)
    o("#define FIXED_TAN_HALFANGLE_NMAX %d" % NMAX)
    o("")
    o("static void (* const _fixed_tan_halfangle_tab[])(nn_ptr, nn_ptr,")
    o("    nn_ptr, nn_srcptr) = {")
    o("    NULL, NULL,")
    o("    " + ", ".join("_fixed_tan_halfangle_%d" % n
                         for n in range(NMIN, NMAX + 1)))
    o("};")
