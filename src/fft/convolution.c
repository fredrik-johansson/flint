...
typedef struct
{
    ...
    slong rvbits;
    slong limbs;
    mpn_negmul_scratch_struct * scr;
    int rows;
}
_negmul_pw_arg_struct;

static void
_negmul_pw_one(_negmul_pw_arg_struct * X, slong j)
{
    mpn_normmod_2expp1(X->ii[j], X->limbs);
    if (X->ii != X->jj)
        mpn_normmod_2expp1(X->jj[j], X->limbs);
    mpn_negmul2(X->C, X->ii[j], X->ii[j], X->jj[j], X->scr);
}

static void
_negmul_pw_worker(void * varg)
{
    _negmul_pw_arg_struct * X = (_negmul_pw_arg_struct *) varg;
    slong s, u, j;
    ...
    else
    {
        for (s = X->j0; s < X->j1; s++)
        {
            ...
            for (u = 0; u < X->srt; u++)
                _negmul_pw_one(X, X->base + t*X->srt + u);
        }
    }
}

static void
_negmul_pw_run(_negmul_pw_arg_struct * proto, slong count)
{
    thread_pool_handle * handles;
    slong nhandles, nthreads, i;
    _negmul_pw_arg_struct * args;
    mpn_negmul_scratch_struct * scr;
    slong j0 = proto->j0;
    ...
    if (nhandles == 0)
    {
        proto->scr = proto->C->scr;
        _negmul_pw_worker(proto);
        flint_give_back_threads(handles, nhandles);
        return;
    }

    args = flint_malloc(nthreads * sizeof(_negmul_pw_arg_struct));
    scr = flint_malloc(nhandles * sizeof(mpn_negmul_scratch_struct));
    for (i = 0; i < nhandles; i++)
        mpn_negmul_scratch_init(scr + i, proto->C);
...
void
_fft_negmul_pointwise(mpn_negmul_ctx_struct * negctx, mp_limb_t ** ii,
                      mp_limb_t ** jj, slong j0, slong j1, slong limbs)
{
    _negmul_pw_arg_struct X = { negctx, ii, jj, j0, j1,
                                0, 0, 0, limbs, NULL, 0 };
    _negmul_pw_run(&X, j1 - j0);
}

void
_fft_negmul_pointwise_rows(mpn_negmul_ctx_struct * negctx,
                      mp_limb_t ** ii, mp_limb_t ** jj, slong base,
                      slong srt, slong nrows, slong rvbits, slong limbs)
{
    _negmul_pw_arg_struct X = { negctx, ii, jj, 0, nrows,
                                base, srt, rvbits, limbs, NULL, 1 };
    _negmul_pw_run(&X, nrows);
}
#else
/* without the fft_small engine no context can exist; these are
   never reached and exist only to satisfy references */
void
_fft_negmul_pointwise(struct mpn_negmul_ctx_struct * negctx,
                      mp_limb_t ** ii, mp_limb_t ** jj, slong j0,
                      slong j1, slong limbs)
{
    FLINT_ASSERT(negctx == NULL);
    (void) negctx; (void) ii; (void) jj;
    (void) j0; (void) j1; (void) limbs;
}

void
_fft_negmul_pointwise_rows(struct mpn_negmul_ctx_struct * negctx,
                      mp_limb_t ** ii, mp_limb_t ** jj, slong base,
                      slong srt, slong nrows, slong rvbits, slong limbs)
{
    FLINT_ASSERT(negctx == NULL);
    (void) negctx; (void) ii; (void) jj; (void) base;
...
void fft_convolution_negmul(struct mpn_negmul_ctx_struct * negctx,
                          ...
                          mp_limb_t ** t2, mp_limb_t ** s1, mp_limb_t ** tt)
{
   ...
   } else
   {
             ...
             < 8*flint_get_num_threads())
      {
         ...
         _fft_negmul_pointwise_rows(negctx, ii, jj, 2*n, sqrt,
                                    (trunc - 2*n)/sqrt,
...
#if FLINT_HAVE_FFT_SMALL
         if (negctx != NULL)
            fft_mfa_truncate_sqrt2_inner_negmul(negctx, ii, jj, n, w,
...
slong fft_negmul_choose_limbs(slong tight, slong n_outer, slong * m_out)
{
#if !FLINT_HAVE_FFT_SMALL
    *m_out = 0;
    ...
    for (ii = 0; ii < (slong) (sizeof(bs) / sizeof(bs[0])); ii++)
    {
        ...
        if ((m * b) % FLINT_BITS != 0)
            continue;
        if (m * b < FLINT_FFT_NEGMUL_MIN_RING)
            continue;
        ...
        np = mpn_negmul_choose_np(get_default_mpn_ctx(), b, depth);
        if (np < 0)
            continue;
