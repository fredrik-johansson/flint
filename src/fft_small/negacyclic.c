...
slong
mpn_negmul_choose_np(mpn_ctx_struct * R, slong b, slong depth)
{
    fmpz_t P;
...
void
mpn_negmul_ctx_init(mpn_negmul_ctx_struct * C, mpn_ctx_struct * R,
                    slong N, slong m)
{
    slong np;

    if (N % m != 0 || m < 16 || (m & (m - 1)) != 0)
        flint_throw(FLINT_ERROR, "mpn_negmul_ctx_init: need N = m*b "
                    "with m a power of two >= 16\n");
    np = mpn_negmul_choose_np(R, N / m, FLINT_BIT_COUNT(m) - 1);
    if (np < 0)
        flint_throw(FLINT_ERROR, "mpn_negmul_ctx_init: no supported "
    ...
    mpn_negmul_scratch_init(C->scr, C);
}
...
/* For a caller-fixed ring 2^N + 1, choose the digit count m over
   ...
   cost staircase. */
void
mpn_negmul_ctx_init_auto(mpn_negmul_ctx_struct * C, mpn_ctx_struct * R,
                         slong N)
{
    ...
    for (ii = 0; ii < (slong) (sizeof(bs) / sizeof(bs[0])); ii++)
    {
        ...
        np = mpn_negmul_choose_np(R, b, depth);
        if (np < 0)
            continue;
    ...
    if (bestm == 0)
        flint_throw(FLINT_ERROR, "mpn_negmul_ctx_init_auto: no "
                    "supported digit splitting for N = %wd\n", N);
    mpn_negmul_ctx_init(C, R, N, bestm);
}

void
mpn_negmul_ctx_clear(mpn_negmul_ctx_struct * C)
{
    ...
    mpn_negmul_scratch_clear(C->scr);
    flint_free(C->crt2);
...
void
mpn_negmul2(mpn_negmul_ctx_struct * C, nn_ptr z, nn_srcptr x,
            nn_srcptr y, mpn_negmul_scratch_struct * S)
{
...
void
mpn_negmul(mpn_negmul_ctx_struct * C, nn_ptr z, nn_srcptr x,
           nn_srcptr y)
{
    mpn_negmul2(C, z, x, y, C->scr);
}

/* per-thread mutable state for mpn_negmul2; the context itself is
   read-only during multiplication and may be shared */
void
mpn_negmul_scratch_init(mpn_negmul_scratch_struct * S,
                        const mpn_negmul_ctx_struct * C)
{
...
void
mpn_negmul_scratch_clear(mpn_negmul_scratch_struct * S)
{
