...
/* Hand-written specialization (r = 16), carried over verbatim by
   dev/tune_fixed.py --pin 16 --emit from dev/log1p_hand.inc; edit
   there, not here. */
...
#ifndef MPN_RIGHT_SHIFT_LOW
#define MPN_RIGHT_SHIFT_LOW(hi, lo, b) \
    (((lo) >> (b)) | ((b) ? ((hi) << (FLINT_BITS - (b))) : UWORD(0)))
#endif
