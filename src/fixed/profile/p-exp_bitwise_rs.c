...
/* Compare fixed_exp_bitwise_rs for several values of the reduction
   parameter r against arb_exp at equal precision. */
...
int main(void)
{
    ...
    /* warm the log table cache to the largest size */
    {
        ...
        fixed_exp_bitwise_rs(y, x, 157, 256);
    }
    ...
    for (a = 0; a < NUM_N; a++)
    {
        ...
        for (ri = 0; ri < NUM_R; ri++)
        {
            BENCH(t, fixed_exp_bitwise_rs(y, x, n, rs[ri]));
        ...
        BENCH(t, arb_exp(e, xa, FLINT_BITS * n));
