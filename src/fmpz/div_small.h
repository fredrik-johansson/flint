...
/*
    Division of a 1- or 2-limb mpz-sized g by a small nonzero h without
    touching the mpz layer (one or two hardware divisions). rnd is 0 for
    truncation, -1 for floor and 1 for ceiling; q and r may each be NULL,
    and may alias g. Returns 1 if the case was handled (|g| has at most two
    limbs), 0 otherwise.
*/
static inline int
_fmpz_div_qr_small_divisor(fmpz_t q, fmpz_t r, const fmpz_t g, slong h, int rnd)
{
    ...
    ulong ah = FLINT_ABS(h), g0, g1, q0, q1, r0;
    int gneg = gn < 0, hneg = h < 0, qneg = gneg ^ hneg, rneg = gneg;
