    ...
    slong rank, const slong * P, const slong * pivs)
{
    ...
    /* nonsingular modulo p, hence nonsingular; the solve cannot fail
       other than through a defect we would want to surface anyway */
...
fmpz_mat_is_singular(const fmpz_mat_t A)
{
    ...
    while (1)
    {
        ...
        /* try the kernel-vector certificate */
        if (_fmpz_mat_certify_singular_lu_mod_p(A, rank, P, pivs))
        {
            result = 1;
            break;
        }
