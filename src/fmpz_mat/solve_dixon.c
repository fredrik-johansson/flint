                                ...
                                const fmpz_mat_t A, const fmpz_t det_bound, slong bits)
{
    ...
    while (1)
    {
        ...
        /* det_bound == NULL: the caller knows that A is nonsingular, so
           no certification is wanted (and none is possible); just keep
           looking for a prime of full rank */
        if (det_bound == NULL)
            continue;
...
{
    return _fmpz_mat_find_good_prime_and_lu2(LU, P, A, det_bound, NMOD_MAT_OPTIMAL_MODULUS_BITS);
}
