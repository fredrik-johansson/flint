...
#define DIXON_MAX_LEVELS 64
    ...
    if (kexp_first > 1 && _fmpz_mat_solve_dixon_use_mm(n, FLINT_ABS(fmpz_mat_max_bits(A))))
        kexp_target = FLINT_MAX(kexp_first, 2 * FLINT_ABS(fmpz_mat_max_bits(A)) / (FLINT_BIT_COUNT(p) - 1));
    fmpz_init(prod);
    fmpz_init(Amax);
    fmpz_init(Bmax);
    fmpz_init(s);
    ...
    while (1)
    {
        ...
        if (kexp < kexp_target && i > warmup
            && FLINT_ABS(fmpz_mat_max_bits(d)) <= bits_A + FLINT_BIT_COUNT(n) + 2 * FLINT_BITS)
        {
               ...
               used in its place. */
            {
                ...
                if (probe_ok)
                {
                    slong rem_bits = bits_A + FLINT_MAX(0, (slong) fmpz_bits(snum_prev) - 4 - FLINT_BIT_COUNT(n * cols))
...

            {
                ...
                /* Precompute residues of A^-1 mod q and A for multimodular
                   ...
                   n ||A||) + 1, |y| < q/2. */
                {
                    slong bits_d = FLINT_MAX(FLINT_ABS(fmpz_mat_max_bits(d)), fmpz_bits(Amax)) + 2;
        ...
        if (fmpz_cmp(ppow, bound) > 0)
        {
            ...
            _fmpz_mat_reconstruct_matwise(Z, den, x, ppow, N, D, t);
            goto dixon_done;
        }
        ...
        if (((probe_ok && (probe_stable || ieff <= 8)) || ieff <= 8) && i >= next_full)
        {
            ...
            if (_fmpz_mat_solve_reconstruct_attempt(Z, den, x, ppow, N, D,
