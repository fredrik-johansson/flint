               ...
               const fmpz * poly2, slong len2, slong nlo, slong nhi)
{
    ...
    limbs = fft_negmul_choose_limbs(limbs, n, &negm);
    size = limbs + 1;
   ...
   for (i = 1; i < N; i++)
   {
...

    {
        ...
        if (negm != 0)
        {
            mpn_negmul_ctx_t negctx;
            mpn_negmul_ctx_init(negctx, get_default_mpn_ctx(),
                                FLINT_BITS*limbs, negm);
            fft_convolution_negmul(negctx, ii, jj, loglen - 2, limbs,
                                   len_out, t1, t2, s1, tt);
            mpn_negmul_ctx_clear(negctx);
        }
        else
#endif
